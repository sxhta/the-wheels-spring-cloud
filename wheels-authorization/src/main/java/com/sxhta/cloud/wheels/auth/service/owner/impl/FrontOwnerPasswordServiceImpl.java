...
import com.sxhta.cloud.wheels.auth.service.owner.FrontOwnerPasswordService;
 ...
 */
@Service
public class FrontOwnerPasswordServiceImpl implements FrontOwnerPasswordService {
    ...
    @Inject
    private FrontOwnerRecordLogService frontOwnerRecordLogService;
    ...
    public void validate(AbstractUserEntity user, String password) {
        ...
        if (retryCount >= maxRetryCount) {
            ...
            frontOwnerRecordLogService.recordLoginInfo(username, Constants.LOGIN_FAIL, errMsg);
            throw new ServiceException(errMsg);
        }

        if (!matches(user, password)) {
            retryCount = retryCount + 1;
            frontOwnerRecordLogService.recordLoginInfo(username, Constants.LOGIN_FAIL, String.format("密码输入错误%s次", retryCount));
